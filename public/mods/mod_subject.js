initDataTable('#dataTable', [{
    ...
    $('.edit').on('click', function () {
        resetInvalid();
        var id = $(this).data('id')
        $('#myModal .modal-title').html('Edit Mata Pelajaran');
        ...
        fetch(`${window.location.href}/${id}/show`)
            ...
            .then(res => {
                if (res.status == 200) {
                    ...
                    $('#myModal').find('#degree').val(res.data.data.degree_id == null ? 'all' : res.data.data.degree_id)
                    $('#myModal').find('#major').val(res.data.data.major_id == null ? 'all' : res.data.data.major_id)
                    $('#myModal').find('#grade').val(res.data.data.grade)
                } else {
...
$('.add').on('click', function () {
    ...
    $('#myModal .modal-title').html('Tambah Mata Pelajaran Baru');
