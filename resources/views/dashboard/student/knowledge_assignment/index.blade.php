...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Tugas Pengetahuan</li>
    </ol>
@endsection
    ...
    <div class="row">
                ...
                <div class="col-md-4 col-sm-6 col-xs-12 col-12">
                    <div class="card shadow-sm mb-3">
                        <div class="card-body">
                            ...
                            {{ $item->name }} <br>
                            @if ($item->knowledge_assessment->count() > 0)
                                @if ($item->knowledge_assessment[0]->score != null)
                                    <span class="badge badge-soft-success p-2">
                                        <strong>Nilai : {{ $item->knowledge_assessment[0]->score }}</strong>
                                    </span>
                                @else
                                        ...
                                        <td>
                                            @foreach ($item->assignmentAttachment as $attachment)
                                                ...
                                                @else
                                                    ...
                                                    @endphp
                                                    @if ($ext == 'pdf')
                                                        <a href="{{ route('students.knowledge-assignment.download', $attachment->attachment) }}"
                                                        ...
                                                        <td>
                                                            @if ($item->knowledge_assessment->count() > 0)
                                                                @can('create-student-dashboard-assignment')
                                                                    @if($item->knowledge_assessment[0]->attachment_type == 'link')
                                                                        <a href="{{ $item->knowledge_assessment[0]->attachment }}" target="_blank" rel="noopener noreferrer">
                                                                            <i class="fa fa-external-link-alt"></i>
                                                                            Lihat Tugas
                                                                        </a>
                                                                        <hr>
                                                                    @else
                                                                        <a href="{{ route('students.knowledge-assignment.download-tugas', $item->knowledge_assessment[0]->attachment) }}"
                                                                            target="_blank"
                                                                            style="margin-right: 10px;white-space: nowrap">
                                                                            @php $exts = explode('.', $item->knowledge_assessment[0]->attachment);$ext = end($exts); @endphp
                                                                            @if($ext == 'pdf')
                                                                                <i class="fas fa-file-pdf text-danger"></i>
                                                                                FILE TUGAS
                                                                            @elseif($ext == 'ppt')
        ...
        @else
            <div class="text-center my-5">
                ...
                <h2 class="mt-5">Tidak ada tugas pengetahuan</h2>
                <p class="mt-3">Tidak ada tugas pengetahuan saat ini!</p>
            </div>
        @endif
    </div>
    ...
    <div class="modal fade" id="myModal" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('students.knowledge-assignment.upload') }}" method="post"
                    enctype="multipart/form-data" data-request="ajax" reload-view="true">
